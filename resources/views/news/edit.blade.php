                    ...
                    <div class="row row-lg">
                        <div class="col-md-12 col-lg-10">
                            <div class="example-wrap">
                                <form class="form-horizontal" method="post" action="{{ route('activity.news.update', $news->id) }}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    @if ($errors->any())
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-7 input-group">
                                            <input type="text" class="form-control" value="{{ old('title') ?? $news->title }}" name="title" placeholder="新闻标题" autocomplete="off">
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label class="col-md-3 col-form-label">跳转地址：</label>
                                        <div class="col-md-7 input-group">
                                            <input type="text" class="form-control" value="{{ old('original_url') ?? $news->original_url }}" name="original_url" placeholder="跳转地址" autocomplete="off">
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label class="col-md-3 col-form-label">封面图片：</label>
                                        <div class="col-md-7">
                                            <input type="file" class="form-control dropify" name="image" data-plugin="dropify" data-default-file="{{ $news->cover_url }}">
                                            <p class="form-text">封面图片 720 * 320</p>
                                            <input type="hidden" name="cover_url" value="{{ $news->cover_url }}">
                                        </div>
                                    </div>
